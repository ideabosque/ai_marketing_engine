...
def data_sync_decorator(original_function):
    ...
    def wrapper_function(*args, **kwargs):
        ...
        if len(args[0].context.get("setting", {}).get("file_attributes", [])) > 0:
            for file_attribute in (
                args[0].context.get("setting", {}).get("file_attributes", [])
            ):
                if (
                    file_attribute in message.get("data", {})
                    and message.get("data", {}).get(file_attribute) is not None
                ):
                    try:
                        download_url = generate_download_url(
                            args[0],
                            **{
                                "object_key": message.get("data", {}).get(
                                    file_attribute
                                )
                            },
                        )
                        message["data"][file_attribute] = {"url": download_url}
                    except Exception as e:
...
def generate_download_url(info: ResolveInfo, **kwargs: Dict[str, Any]) -> str:
