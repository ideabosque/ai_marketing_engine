...
``pg_table_prefix`` setting (e.g. ``"ame_"``). Models call ``prefixed_table``
in a ``declared_attr`` ``__tablename__`` so the prefix is applied when the
class is defined. Models must be imported **after** ``Base.table_prefix`` is
configured — the ``_import_all_models`` flow in ``utils.py`` guarantees this
because ``Config.initialize`` runs before table creation.
"""
...
try:
    ...
    raise ImportError(
        ...
        "Install with: pip install ai-marketing-engine[postgresql]"
    )
...
def _serialize_value(val: Any) -> Any:
    ...
    if isinstance(val, (datetime.datetime, datetime.date)):
        ...
        # requires a datetime instance, not a str.
        return val
