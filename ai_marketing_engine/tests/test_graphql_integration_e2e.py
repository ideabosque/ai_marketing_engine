...
TOKEN = uuid4().hex[:10]
REPORT = Path("docs/test_results") / f"live_integration_results_{RUN_ID}.md"
...
SETTINGS = {
    ...
    "initialize_tables": 1,
    "module_zip_path": "C:/tmp/adaptor_zips",
    "module_extract_path": "C:/tmp/adaptors",
}
SAFE_SETTINGS = {**SETTINGS, "db_password": "<redacted>"}


logging.basicConfig(level=logging.INFO)
logger = logging.getLogger("ame_live_graphql")
Config._initialized = False
engine = AIMarketingEngine(logger, **SETTINGS)
records = []
ids = {}
...
def call(sid, model, op, kind, doc, variables=None, tenant=None, expect_error=False, skip_reason=None):
    ...
    try:
        raw = engine.ai_marketing_graphql(query=doc, variables=variables, context=ctx)
        elapsed = round((time.perf_counter() - start) * 1000, 2)
        output = parse(raw)
        body = output.get("body", {})
        errors = body.get("errors") if isinstance(body, dict) else None
        status = "pass" if ((expect_error and errors) or (not expect_error and not errors)) else "fail"
        rec = {
            "sid": sid,
            "model": model,
            "operation": op,
...
def write_report():
    ...
    lines = [
        "# Live Integration Results - ai_marketing_engine GraphQL SOP 2.1.0",
        "",
        f"- Generated at: `{datetime.now().isoformat()}`",
        "- Project / module: `ai_marketing_engine`",
        "- Business domain: `ecommerce`",
        "- Environment target: `dev (local)`",
        "- Gateway / base URL: unavailable; SOP uses in-process dispatch",
        "- Endpoint: `gpt`",
        "- Partition / namespace: `neprodai`",
        "- Interface URL: `ai_marketing_engine.main:dispatch_graphql`",
        "- SOP reference: `docs/integration_scenarios_sop.md`, version `2.1.0` (confirmed by user message)",
        ...
        "## Executive Summary",
        "",
        "This run executed the confirmed GraphQL-only SOP through `dispatch_graphql` against local PostgreSQL. The runner initialized the PostgreSQL backing schema with SQLAlchemy `create_all(checkfirst=True)`, executed GraphQL operations in dependency order, recorded every live GraphQL input and output, and attempted reverse-order GraphQL cleanup for generated test data.",
        "",
        "## Function Results",
        "",
    ]
